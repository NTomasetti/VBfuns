...
arma::mat sobolPoints(int N, int D, int skip, std::string fileLocation) {
  using namespace std;
  // We want to be able to skip the first few rows as they have stronger patterns and look less random
  N += skip;
  std::ifstream infile(fileLocation ,ios::in);
  if (!infile) {
    Rcpp::Rcout << "Input file containing direction numbers cannot be found!\n";
  ...
  arma::mat points(N, D, arma::fill::zeros);
