...
#' @param data Data passed directly into model.
#' @param lambda Parameter vector to be optimised. If lambda is a matrix, each column is assumed to be the parameter vector for a component of a mixture of multivariate gaussians, with the first row corresponding to unnormalised weights z, where the normalised weights are given by p = exp(z) = sum(exp(z))
