...
#' @param sigma the length n vector of standard deviations.
...
#' @param supports a matrix with n columns that gives the support of each variable (per column) to calculate the densities over. If NULL, automatically find a suitable density.
#' @export
gaussianDensity <- function(mu, sigma, transform = NULL, names = NULL, supports = NULL){
  if(!is.vector(mu)){
